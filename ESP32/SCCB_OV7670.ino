byte readRegister(byte IdAdress, byte SubAdress, int SCL, int SDA, int Delay){
    ...
    for (int phase = 1; phase <3; phase++){
          if (!writeByte(Message[phase - 1], SCL, SDA, Delay)){
              Serial.print("Don't care bit error during phase ");
              Serial.print(phase);
              Serial.println(" of two phase writing transmission");
            }
      }
     ...
     if (!writeByte(IdAdress, SCL, SDA, Delay)){
              Serial.println("Don't care bit error during phase 1 of two phase reading transmission");
            }
